...
{ # @author Peter Carl
  # @author Matthieu Lestel
	...
	#result = sqrt(sum((r - MAR)^p)/len) in the already coded function which 
        #doesn't matter when potential=FALSE since p=2 but leads to take the sqrt 
        #of a negative number when potential=TRUE and p=1
        return(result)
	reclass(result, R0)
    }
    else {
